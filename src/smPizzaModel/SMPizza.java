...
public class SMPizza extends AOSimulationModel {
	...
	// Constructor
	public SMPizza(double t0time, double tftime, int numPersons, int totalDrivers, int loadareaSize, Seeds sd,
			boolean flg) {
		traceFlag = flg;
	...
	public void eventOccured() {
		...
		if (traceFlag) {
			System.out.println();
			System.out.println("__________________________________CLOCK: " + getClock());
			System.out.println("Make Table Positions: " + Arrays.toString(rqMakeTable.position));
			System.out.println("Techphone length: " + qTechphone.size());
			System.out.println("Slide Length: " + qSlide.size());
			System.out.println("Load Area Used: " + rgLoadArea.usedSpace + "/" + rgLoadArea.size);
